 ...
 * （1）如果第i天不出行，则 ：algorithms.dp(i) = algorithms.dp(i-1)
 * （2）如果第i天出行，则：algorithms.dp(i) = min { algorithms.dp(i-1) + costOne , algorithms.dp(i-7) + costSeven , algorithms.dp(i-30) + costThirty }
 *
 *  algorithms.dp(最后的出行日)就是要求的解
 */
class Solution983 {
