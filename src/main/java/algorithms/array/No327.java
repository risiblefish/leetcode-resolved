 ...
 * 结合代码98-103行来看，归并排序厉害之处在于，每个数至多和其他数比较1次，并且它能够避开无效比较(比如 a < b, b < c , 那么a < c也就是已知了，不需要再比较a,c)
 *
 * 再举个例子，
 * 下标           0 1 2 3 4  5  6 7
 * 原数组         2 3 3 1 2  4 -9 1
 * 前缀和数组     2,5,8,9,11,15 6,7
 * [lower,upper] = [-1,2]
 * 我们就盯着前缀和数组最右边的7来看
 * 第1次merge， 是L=[7] R=[7], 也就是7自身，表示0-7的区间和，它不在[-1,2]里
 * 第2次merge，是L=[6] R=[7]， 左段的值需要落在 [7-2, 7-(-1)]即[5,8]区间，显然6在这个区间，所以找到1个，表示7-7范围的区间和
 * 第3次merge，是L=[11,15] R=[6,7]，对于右段的7而言，依次看左段的每一个前缀和是否落在[5,8]，显然11，15都不满足
 * 第4次merge, L=[2,5,8,9] R=[6,7,11,15]，对于右段的7而言，依次看左段的2，5，8，9是否在[5,8]里，显然5，8满足，所以找到2个，分别表示5-11范围的区间和
 * 至此，前缀数组合并完成 变为 [2,5,6,7,8,9,11,15]
 * 我们发现，每次和7比较的左组中的数，在比较完成之后，都会放到右组中去，下一次merge时，7只会和左组中的数进行比较，
 */
class Solution327 {
    long[] preSum, tmp;
    int lower, upper;
